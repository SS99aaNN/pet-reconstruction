...
def generate_norm_file():
    os.chdir(r"/path/to/env")
    scanner_option = ScannerOption("PET_11panel_LD")
    ac_factor = np.fromfile(r"/path/to/acf_file", dtype=np.float32)
    op_path = r"/path/to/output"
    num_counts = 0
...
def add_acf_to_norm_file():
    ...
    scanner_option = ScannerOption("PET_11panel_LD")
    ac_factor = np.fromfile(r"/path/to/acf_file", dtype=np.float32)
    norm_file_path = r"/path/to/norm_file"
    op_path = r"/path/to/output"
